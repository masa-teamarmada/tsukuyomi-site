...
const C = {
  bg:      "#f7f3ec",   // 和紙クリーム
  bgAlt:   "#ede7dc",   // やや濃い和紙
  bgWarm:  "#faf7f2",   // 最も薄い和紙（ステップ等）
  border:  "#d8cfc4",   // 温かみのあるボーダー
  text:    "#1e1a16",   // 濃い墨色テキスト
  muted:   "#6b5e52",   // 薄墨
  faint:   "#b0a89e",   // 極薄
  gold:    "#8a6318",   // 和金色
  goldLt:  "#f5edda",   // 金の薄い背景
  red:     "#8b2c24",   // 朱色
  redLt:   "#f9edeb",   // 朱の薄い背景
  blue:    "#2a5fa3",   // 藍青（道路色）
  blueLt:  "#e8eef8",   // 藍青の薄い背景
  green:   "#3a6e45",   // 萌葱（鉄道色）
  greenLt: "#e8f3eb",   // 萌葱の薄い背景
};
...
const features = [
  {
    icon: "🛣️",
    title: "道路制覇",
    description: "国道・都道府県道・高速道路を走るたびに地図が塗られる。125万以上のセグメントに対応。",
    accent: C.blue,
    accentLt: C.blueLt,
  },
  {
    icon: "🚃",
    title: "鉄道路線制覇",
    description: "乗車するたびに路線が埋まっていく。全国の鉄道路線を網羅。",
    accent: C.green,
    accentLt: C.greenLt,
  },
  {
    icon: "📍",
    title: "GPS自動記録",
    description: "起動しておくだけで道路を自動検出。手入力は不要。",
    accent: C.red,
    accentLt: C.redLt,
  },
  {
    icon: "📦",
    title: "オフライン対応",
    description: "道路データ（178MB）はデバイス内に搭載。山奥でも電波圏外でも確実に記録。",
    accent: C.gold,
    accentLt: C.goldLt,
  },
  {
    icon: "🔒",
    title: "プライバシー設計",
    description: "GPS生ログはデバイス内のみ。クラウドに送るのは制覇フラグだけ。",
    accent: C.blue,
    accentLt: C.blueLt,
  },
  {
...
const steps = [
  {
    ...
    num: "01",
    title: "アプリを起動する",
    desc: "旅行・ドライブ・電車移動の前に起動するだけ。位置情報の許可だけ設定すればいい。",
  },
  {
    kanji: "二",
    num: "02",
    title: "いつも通り移動する",
    desc: "あとは普段の移動と変わらない。GPSが自動で道路・路線を検出して記録し続ける。",
  },
  {
    kanji: "三",
    num: "03",
    title: "地図が塗られていく",
    desc: "帰宅後に地図を開くと、走った道が色づいている。日本が少しずつ完成していく。",
  },
];
...
export default function MichinoriPage() {
  return (
    ...
    >
      {/* ── Header ── */}
      <header
        style={{ borderBottom: `1px solid ${C.border}`, background: "rgba(247,243,236,0.92)", backdropFilter: "blur(10px)" }}
        className="px-6 py-4 flex items-center justify-between sticky top-0 z-10"
      >
        <div className="flex items-center gap-2">
          <span style={{ color: C.red, fontSize: "18px" }}>⛩</span>
          <span className="text-sm font-bold" style={{ color: C.text, letterSpacing: "0.12em" }}>
            みちのり
          </span>
        </div>
        <nav className="flex gap-5 text-xs" style={{ color: C.muted }}>
          <Link href="/michinori/support" className="hover:opacity-60 transition-opacity">
            サポート
          </Link>
          <Link href="/michinori/privacy" className="hover:opacity-60 transition-opacity">
            プライバシー
          </Link>
        </nav>
      </header>
      ...
      <section className="px-6 py-16 sm:py-28">
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            <p
              className="text-xs tracking-[0.35em] mb-6 font-medium inline-flex items-center gap-2 px-3 py-1 rounded-full"
            ...
            <p className="text-base leading-loose mb-10" style={{ color: C.muted, maxWidth: "420px" }}>
              道路・鉄道路線の制覇に特化したiOSアプリ。
              <br />
              走るたびに地図が塗られ、日本が少しずつ埋まっていく。
            </p>
            <div className="flex flex-wrap items-center gap-4">
              {/* App Store バッジ（グレーアウト） */}
              <span
            ...
            <div className="relative" style={{ width: "230px" }}>
              ...
              <div
                style={{
                  position: "absolute",
                  inset: "-24px",
                  background: `radial-gradient(ellipse at center, ${C.blueLt} 0%, transparent 70%)`,
                  borderRadius: "50%",
                  pointerEvents: "none",
                }}
              />
              ...
              <div
                style={{
                  ...
                  borderRadius: "40px",
                  padding: "10px",
                  boxShadow: "0 20px 60px rgba(30,26,22,0.18), 0 4px 12px rgba(30,26,22,0.10)",
                }}
              >
                {/* ノッチ */}
                  ...
                  {[
                    ...
                    <div key={tab.label} style={{ textAlign: "center" }}>
                      ...
                      <div
                        style={{
                          fontSize: "8px",
                          color: tab.active ? C.blue : C.faint,
                          marginTop: "2px",
      ...
      >
        <div className="max-w-3xl mx-auto grid grid-cols-3 gap-6 text-center">
          {[
            ...
            <div key={s.sub}>
              <p className="text-3xl sm:text-4xl font-bold mb-1.5" style={{ color: s.accent }}>
                {s.value}
              </p>
      ...
      <section className="px-6 py-16 sm:py-20" style={{ background: C.bg }}>
        <div className="max-w-3xl mx-auto">
          <p className="text-xs tracking-[0.3em] mb-1.5 font-medium" style={{ color: C.red }}>
            FEATURES
          </p>
          <h2 className="text-2xl font-bold mb-10" style={{ color: C.text }}>
            できること
          </h2>
              ...
              <div key={item.kanji} className="flex gap-6 items-stretch">
                  ...
                  {idx < steps.length - 1 && (
                    <div
                      style={{
                        ...
                        flex: 1,
                        minHeight: "32px",
                        background: `linear-gradient(to bottom, ${C.gold}40, ${C.border})`,
                        margin: "6px 0",
                      }}
                    />
                  )}
                ...
                <div style={{ paddingBottom: idx < steps.length - 1 ? "28px" : "0" }}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-xs tabular-nums font-mono" style={{ color: C.faint }}>
                      {item.num}
                    </span>
      ...
      <section style={{ background: C.bg }} className="px-6 py-16 sm:py-20">
        <div className="max-w-3xl mx-auto">
          ...
          >
            ...
            <div>
              <p className="text-xs tracking-[0.3em] mb-1.5 font-medium" style={{ color: C.blue }}>
                PRIVACY
              </p>
              ...
              <p className="text-sm leading-loose mb-4" style={{ color: C.muted }}>
                GPS生データ（いつ・どこを移動したか）はデバイス内にのみ保存されます。
                クラウドに送られるのは「この区間を通過した」という制覇フラグのみ。
                詳細な移動履歴が外部に出ることはありません。
              </p>
              <Link
                href="/michinori/privacy"
      ...
      {/* ── Footer ── */}
      <footer style={{ borderTop: `1px solid ${C.border}`, background: C.bgAlt }} className="px-6 py-8">
